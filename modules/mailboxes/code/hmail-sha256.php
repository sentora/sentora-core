...
if (self::$update) {
    if (!fs_director::CheckForEmptyValue($password)) {
		...
		// NEW CODE :Encrypt Users password with Salt and SH256
			...
			$strTemp = $randomString.$strPassword;
			// SHA256 the temp string
			$strHash = hash( 'sha256', $strTemp );
			unset( $strTemp );
			// The final string to be stored
			$strHashedPass = $randomString.$strHash;
			//echo $strHashedPass;
			
			$genpassword = $strHashedPass;        
		// NEW CODE END:
		
		//$password = md5($password);
        $sql->bindParam(':password', $genpassword);
        $sql->bindParam(':mb_address_vc', $rowmailbox['mb_address_vc']);
        $sql->execute();
    }
