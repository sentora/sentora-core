 ...
 */
		...
		$forwarders = self::getForwardList();
		if (!fs_director::CheckForEmptyValue($forwarders)){
			foreach ($forwarders as $forwarder){
				if (!fs_director::CheckForEmptyValue($controller->GetControllerRequest('FORM', 'inDelete_'.$forwarder['id'].''))) {
		        	$result = $mail_db->query("SELECT accountaddress FROM hm_accounts WHERE accountaddress='" . $forwarder['address'] . "'")->Fetch();
					if ($result) {
						$sql = "UPDATE hm_accounts SET accountforwardenabled='0', accountforwardaddress='', accountforwardkeeporiginal='0' WHERE accountaddress='" . $forwarder['address'] . "'";
