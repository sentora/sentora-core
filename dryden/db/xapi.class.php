...
class db_xapi {
	...
	global $zdbh;
		...
		while($row = $sql->fetch()) {
			...
			$new_file .= "# CRONTAB FOR ZPANEL CRON MANAGER MODULE                                        #".fs_fh::NewLine();
			$new_file .= "# Module Developed by Bobby Allen, 17/12/2009                                   #".fs_fh::NewLine();
			$new_file .= "# Automatically generated by ZPanel ".ctrl_options::GetOption('zpanel_version')." ".date('M-d-Y h:m:s').fs_fh::NewLine();
			$new_file .= "#################################################################################".fs_fh::NewLine();
			$new_file .= "# WE DO NOT RECOMMEND YOU MODIFY THIS FILE DIRECTLY, PLEASE USE ZPANEL INSTEAD! #".fs_fh::NewLine();
			$new_file .= "#################################################################################".fs_fh::NewLine();
			$new_file .= "# CronW Debug infomation can be found in this file here:-                       #".fs_fh::NewLine();
			$new_file .= "# C:\WINDOWS\System32\crontab.txt                                               #".fs_fh::NewLine();
			$new_file .= "#################################################################################".fs_fh::NewLine();
			$new_file .= "0 * * * * ".ctrl_options::GetOption('php_exer')." C:\ZPanel\panel\daemon.php".fs_fh::NewLine();
			$new_file .= "#################################################################################".fs_fh::NewLine();
			$new_file .= "# DO NOT MANUALLY REMOVE ANY OF THE CRON ENTRIES FROM THIS FILE, USE ZPANEL     #".fs_fh::NewLine();
			$new_file .= "# INSTEAD! THE ABOVE ENTRIES ARE USED FOR ZPANEL TASKS, DO NOT REMOVE THEM!     #".fs_fh::NewLine();
			$new_file .= "#################################################################################".fs_fh::NewLine();
	...
	global $zdbh;
		...
		$new_file  = "################################################################".fs_fh::NewLine();;
		$new_file .= "# Apache VHOST configuration file".fs_fh::NewLine();
		$new_file .= "# Automatically generated by ZPanel ".ctrl_options::GetOption('zpanel_version')." ".date('M-d-Y h:m:s').fs_fh::NewLine();
		$new_file .= "################################################################".fs_fh::NewLine();
		$new_file .= fs_fh::NewLine();		
		...
		$new_file .= "<Directory \"".ctrl_options::GetOption('zpanel_root')."\">".fs_fh::NewLine();
