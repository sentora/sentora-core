...
if ($d_absissa_enabled == "true") {
    ...
    foreach ($Values as $key => $Value) {
        if ($Value == "") {
            $Value = VOID;
        } $myData->addPoints($Value, "Absissa");
    }
...
if (isset($Axis[1])) {
    ...
    if ($Mode == "Source") {
        if ($d_axis1_position == "left") {
            echo '$myData->setAxisPosition(1,AXIS_POSITION_LEFT);' . "\r\n";
        } else {
            echo '$myData->setAxisPosition(1,AXIS_POSITION_RIGHT);' . "\r\n";
        }
...
if (isset($Axis[2])) {
    ...
    if ($Mode == "Source") {
        if ($d_axis2_position == "left") {
            echo '$myData->setAxisPosition(2,AXIS_POSITION_LEFT);' . "\r\n";
        } else {
            echo '$myData->setAxisPosition(2,AXIS_POSITION_RIGHT);' . "\r\n";
        }
...
if ($Mode == "Render") {
    if ($g_border == "true") {
        $myPicture->drawRectangle(0, 0, $g_width - 1, $g_height - 1, array("R" => 0, "G" => 0, "B" => 0));
    }
    if ($g_shadow == "true") {
...
if ($s_x_labeling == "LABELING_ALL") {
    $Labeling = 691011;
} else {
    $Labeling = 691012;
}
if ($s_mode == "SCALE_MODE_FLOATING") {
    $iMode = 690201;
}
if ($s_mode == "SCALE_MODE_START0") {
    $iMode = 690202;
}
if ($s_mode == "SCALE_MODE_ADDALL") {
    $iMode = 690203;
}
if ($s_mode == "SCALE_MODE_ADDALL_START0") {
    $iMode = 690204;
}
...
if ($s_x_skip != 0) {
    $Settings["LabelSkip"] = $s_x_skip;
}
if ($s_cycle_enabled == "true") {
    $Settings["CycleBackground"] = TRUE;
}
if ($s_arrows_enabled == "true") {
    $Settings["DrawArrows"] = TRUE;
}
if ($s_grid_x_enabled == "true") {
    $Settings["DrawXLines"] = TRUE;
} else {
    $Settings["DrawXLines"] = 0;
}
...
if ($Mode == "Render") {
    if ($s_grid_y_enabled == "true") {
        $Settings["DrawYLines"] = ALL;
    } else {
        $Settings["DrawYLines"] = NONE;
    }
...
if ($Mode == "Render") {
    ...
    if ($g_shadow == "true") {
        echo '$myPicture->setShadow(TRUE,array("X"=>1,"Y"=>1,"R"=>50,"G"=>50,"B"=>50,"Alpha"=>10));' . "\r\n\r\n";
    }
}

/* Chart specific parameters -------------------------------------------------------------------------------- */
if ($c_display_values == "true") {
...
if ($c_family == "bar") {
    if ($c_bar_rounded == "true") {
        $Config["Rounded"] = TRUE;
    }
    if ($c_bar_gradient == "true") {
        $Config["Gradient"] = TRUE;
    }
    if ($c_around_zero1 == "true") {
...
if ($l_enabled == "true") {
    ...
    if ($l_format == "LEGEND_NOBORDER") {
        $Config["Style"] = 690800;
    }
    if ($l_format == "LEGEND_BOX") {
        $Config["Style"] = 690801;
    }
...
function getTextAlignCode($Mode) {
    if ($Mode == "TEXT_ALIGN_TOPLEFT") {
        return(690401);
    }
    if ($Mode == "TEXT_ALIGN_TOPMIDDLE") {
        return(690402);
    }
    if ($Mode == "TEXT_ALIGN_TOPRIGHT") {
        return(690403);
    }
    if ($Mode == "TEXT_ALIGN_MIDDLELEFT") {
        return(690404);
    }
    if ($Mode == "TEXT_ALIGN_MIDDLEMIDDLE") {
        return(690405);
    }
    if ($Mode == "TEXT_ALIGN_MIDDLERIGHT") {
        return(690406);
    }
    if ($Mode == "TEXT_ALIGN_BOTTOMLEFT") {
        return(690407);
    }
    if ($Mode == "TEXT_ALIGN_BOTTOMMIDDLE") {
        return(690408);
    }
    if ($Mode == "TEXT_ALIGN_BOTTOMRIGHT") {
        return(690409);
    }
}
