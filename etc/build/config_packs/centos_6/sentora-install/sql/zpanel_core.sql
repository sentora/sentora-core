...
CREATE TABLE `x_ftpaccounts` (
  ...
  `ft_user_vc` varchar(32) DEFAULT NULL,
...
insert  into `x_modules`(`mo_id_pk`,`mo_category_fk`,`mo_name_vc`,`mo_version_in`,`mo_folder_vc`,`mo_type_en`,`mo_desc_tx`,`mo_installed_ts`,`mo_enabled_en`,`mo_updatever_vc`,`mo_updateurl_tx`) values (48,3,'Protected Directories',200,'protected_directories','user','Password protect your web applications and directories.',NULL,'true','','');
...
insert  into `x_permissions` (`pe_id_pk`,`pe_group_fk`,`pe_module_fk`) values (92,1,48);
insert  into `x_permissions` (`pe_id_pk`,`pe_group_fk`,`pe_module_fk`) values (93,2,48);
insert  into `x_permissions` (`pe_id_pk`,`pe_group_fk`,`pe_module_fk`) values (94,3,48);
...
insert  into `x_settings`(`so_id_pk`,`so_name_vc`,`so_cleanname_vc`,`so_value_tx`,`so_defvalues_tx`,`so_desc_tx`,`so_module_vc`,`so_usereditable_en`) values (18,'news_url','ZPanel News API URL','http://api.sentora.org/latestnews.json',NULL,'Zpanel News URL','ZPanel Config','false');
insert  into `x_settings`(`so_id_pk`,`so_name_vc`,`so_cleanname_vc`,`so_value_tx`,`so_defvalues_tx`,`so_desc_tx`,`so_module_vc`,`so_usereditable_en`) values (19,'update_url','ZPanel Update API URL','http://api.sentora.org/latestversion.json',NULL,'Zpanel Update URL','ZPanel Config','false');
...
insert  into `x_settings`(`so_id_pk`,`so_name_vc`,`so_cleanname_vc`,`so_value_tx`,`so_defvalues_tx`,`so_desc_tx`,`so_module_vc`,`so_usereditable_en`) values (121,'sentora_port','Sentora Apache Port','80',NULL,'Sentora Apache panel port','ZPanel Config','true');
...
DROP TABLE IF EXISTS `x_zvps_htpasswd_file`;
...
DROP TABLE IF EXISTS `x_zvps_htpasswd_mapper`;
...
DROP TABLE IF EXISTS `x_zvps_htpasswd_user`;
