...
foreach ($tables as $keyname => $each_table) {
    ...
    switch ($each_table['ENGINE']) {
        ...
        case 'Maria' :
            if ($db_is_information_schema) {
                $each_table['Rows'] = PMA_Table::countRecords($db, $each_table['Name']);
            }
        ...
        case 'InnoDB' :
            ...
            if ($each_table['TABLE_ROWS'] < $GLOBALS['cfg']['MaxExactCount']) {
                ...
                $each_table['TABLE_ROWS'] = PMA_Table::countRecords($db, $each_table['TABLE_NAME'], $force_exact = true, $is_view = false);
            } else {
    ...
    if (!$db_is_information_schema) {
        ...
        $drop_message = sprintf(
                $table_is_view ? __('View %s has been dropped') : __('Table %s has been dropped'), str_replace(' ', '&nbsp;', htmlspecialchars($each_table['TABLE_NAME'])));
    }
    ...
    <?php if (!$db_is_information_schema) { ?>
        ...
        ?>&amp;reload=1&amp;purge=1&amp;sql_query=<?php echo urlencode($drop_query); ?>&amp;message_to_show=<?php echo urlencode($drop_message); ?>" >
