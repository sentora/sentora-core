...
echo '<h2>' . "\n"
 . ($GLOBALS['cfg']['MainPageIconic'] ? '<img class="icon" src="' . $GLOBALS['pmaThemeImage'] .
...
<table id="serverstatustraffic" class="data">
    ...
    <tbody>
        <tr class="noclick odd">
            ...
            <td class="value"><?php
    echo
    implode(' ', PMA_formatByteDown($server_status['Bytes_received'], 2, 1));
    ?></td>
            <td class="value"><?php
    echo
    implode(' ', PMA_formatByteDown(
    ...
    ?></td>
        ...
        <tr class="noclick even">
            ...
            <td class="value"><?php
    echo
    implode(' ', PMA_formatByteDown($server_status['Bytes_sent'], 2, 1));
    ?></td>
            <td class="value"><?php
    echo
    implode(' ', PMA_formatByteDown(
    ...
    <tbody>
        <tr class="noclick odd">
            ...
            <td class="value"><?php echo
                PMA_formatNumber($server_status['Max_used_connections'], 0);
    ?>  </td>
            <td class="value">--- </td>
            <td class="value">--- </td>
        </tr>
        <tr class="noclick even">
            ...
            <td class="value"><?php echo
                PMA_formatNumber($server_status['Aborted_connects'], 4, 0);
    ?></td>
            <td class="value"><?php
                echo
                PMA_formatNumber($server_status['Aborted_connects'] * $hour_factor, 4, 2);
    ?></td>
            <td class="value"><?php
                echo
                $server_status['Connections'] > 0 ? PMA_formatNumber(
                                $server_status['Aborted_connects'] * 100 / $server_status['Connections'], 0, 2) . '%' : '--- ';
    ?></td>
        </tr>
        <tr class="noclick odd">
            <th class="name"><?php echo __('Aborted'); ?></th>
            <td class="value"><?php echo
                PMA_formatNumber($server_status['Aborted_clients'], 4, 0);
    ?></td>
            <td class="value"><?php
                echo
                PMA_formatNumber($server_status['Aborted_clients'] * $hour_factor, 4, 2);
    ?></td>
            <td class="value"><?php
                echo
                $server_status['Connections'] > 0 ? PMA_formatNumber(
                                $server_status['Aborted_clients'] * 100 / $server_status['Connections'], 0, 2) . '%' : '--- ';
    ?></td>
        </tr>
        <tr class="noclick even">
            <th class="name"><?php echo __('Total'); ?></th>
            <td class="value"><?php echo
                PMA_formatNumber($server_status['Connections'], 4, 0);
    ?></td>
            <td class="value"><?php
                echo
                PMA_formatNumber($server_status['Connections'] * $hour_factor, 4, 2);
    ?></td>
            <td class="value"><?php echo
                PMA_formatNumber(100, 0, 2);
    ?>%</td>
        </tr>
    </tbody>
</table>
...
<h3 id="serverstatusqueries"><?php
                echo
                sprintf(__('<b>Query statistics</b>: Since its startup, %s queries have been sent to the server.'), PMA_formatNumber($server_status['Questions'], 0));
...
<table id="serverstatusqueriessummary" class="data">
    ...
    <tbody>
        <tr class="noclick odd">
            <td class="value"><?php echo
    PMA_formatNumber($server_status['Questions'], 4, 0);
    ?></td>
            <td class="value"><?php
    echo
    PMA_formatNumber($server_status['Questions'] * $hour_factor, 3, 2);
    ?></td>
            <td class="value"><?php
    echo
    PMA_formatNumber(
            $server_status['Questions'] * 60 / $server_status['Uptime'], 3, 2);
    ?></td>
            <td class="value"><?php
    echo
    PMA_formatNumber(
            $server_status['Questions'] / $server_status['Uptime'], 3, 2);
    ?></td>
        </tr>
    </tbody>
</table>
...
// but is included in Questions. Then the total of the percentages is 100.
    ...
    ?>
                <tr class="noclick <?php echo $odd_row ? 'odd' : 'even'; ?>">
                    ...
                    <td class="value"><?php echo
            PMA_formatNumber($value * $hour_factor, 3, 3);
    ?></td>
                    <td class="value"><?php echo
            PMA_formatNumber($value * $perc_factor, 0, 2);
            ?>%</td>
                </tr>
                        <?php
                    }
...
foreach ($sections as $section_name => $section) {
    if (!empty($section['vars'])) {
        ...
        ?>"
            ...
            <?php
            echo
            ($GLOBALS['cfg']['MainPageIconic'] ? '<img src="' . $GLOBALS['pmaThemeImage'] .
