...
abstract class PMA_pChart_chart extends PMA_chart {
    ...
    protected function init() {
        ...
        foreach ($this->getColors() as $key => $color) {
            $this->chart->setColorPalette(
                    $key, hexdec(substr($color, 1, 2)), hexdec(substr($color, 3, 2)), hexdec(substr($color, 5, 2))
            );
        }
    ...
    protected function drawCommon() {
        $this->chart->drawGraphAreaGradient(
                $this->getBgColor(RED), $this->getBgColor(GREEN), $this->getBgColor(BLUE),
    ...
    protected function drawTitle() {
        ...
        $this->chart->drawTextBox(
                0, 0, $this->getWidth(), $this->getLabelHeight(), $this->getTitleText(), 0, $this->getTitleColor(RED), $this->getTitleColor(GREEN), $this->getTitleColor(BLUE), ALIGN_CENTER, false, $this->getTitleBgColor(RED), $this->getTitleBgColor(GREEN), $this->getTitleBgColor(BLUE)
        );
    }
    ...
    protected function setGraphAreaDimensions() {
        $this->chart->setGraphArea(
                $this->getAreaMargin(LEFT), $this->getLabelHeight() + $this->getAreaMargin(TOP), $this->getWidth() - $this->getAreaMargin(RIGHT), $this->getHeight() - $this->getAreaMargin(BOTTOM)
        );
    }
    ...
    protected function drawGraphArea() {
        ...
        $this->chart->drawScale(
                $this->dataSet->GetData(), $this->dataSet->GetDataDescription(), $this->getScale(), $this->getScaleColor(RED), $this->getScaleColor(GREEN), $this->getScaleColor(BLUE), TRUE, 0, 2, TRUE
        );
        ...
        $this->chart->drawGrid(
                4, TRUE, $this->getGridColor(RED), $this->getGridColor(GREEN), $this->getGridColor(BLUE), 20
        );
    }
