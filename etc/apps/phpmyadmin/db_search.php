...
if (isset($_REQUEST['submit_search'])) {
    ...
    function PMA_getSearchSqls($table, $field, $search_str, $search_option) {
        ...
        // Fields to select
        $tblfields = PMA_DBI_fetch_result('SHOW FIELDS FROM ' . PMA_backquote($table) . ' FROM ' . PMA_backquote($GLOBALS['db']), null, 'Field');
...
// end of the "PMA_getSearchSqls()" function
    ...
    foreach ($tables_selected as $each_table) {
        ...
        . '<td>' . sprintf(_ngettext('%s match inside table <i>%s</i>', '%s matches inside table <i>%s</i>', $res_cnt), $res_cnt, htmlspecialchars($each_table)) . "</td>\n";
