...
function rcube_init_settings_tabs()
{
    ...
    $(tab + '> a').removeAttr('onclick').click(function() {
        return false;
    });
}
...
function rcube_mail_ui()
{
    this.popups = {
        ...
        mailboxmenu:    {
            id:'mailboxoptionsmenu', 
            above:1
        },
        composemenu:    {
            id:'composeoptionsmenu', 
            editable:1, 
            overlap:1
        },
...
rcube_mail_ui.prototype = {
    ...
    show_popup: function(popup, show, config)
    {
        ...
        if (!this.popups[popup] && (obj = $('#'+popup)) && obj.length)
            this.popups[popup] = $.extend(config, {
                id: popup, 
                obj: obj
            });
    ...
    show_popupmenu: function(popup, show)
    {
        ...
        if (show && ref) {
            ...
            obj.css({
                left:pos.left, 
                top:(pos.top + (above ? -obj.height() : ref.offsetHeight))
            });
        }
    ...
    searchmenu: function(show)
    {
        ...
        if (show && ref) {
            ...
            obj.css({
                left:pos.left, 
                top:(pos.top + ref.offsetHeight + 2)
                });
...
function rcube_init_mail_ui()
{
    ...
    if (rcmail.env.task == 'mail') {
        ...
        rcmail.addEventListener('afterupload-photo', function(){
            rcmail_ui.show_popup('uploadform', false);
        });
    }
}
...
function rcube_render_mailboxlist()
{
    ...
    list.each(function(){
        ...
        elem.contents().filter(function(){
            return (this.nodeType == 3);            }).get(0).data = abbrev;
    });
}
