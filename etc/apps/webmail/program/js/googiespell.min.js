...
try{netscape.security.PrivilegeManager.enablePrivilege("UniversalBrowserRead")}catch(c){}this.main_controller&&$(this.spell_span).unbind("click");this.orginal_text=$(this.text_area).val()}};this.parseResult=function(a){var b=/\w+="(\d+|true)"/g,c=/\t/g;a=a.match(/<c[^>]*>[^<]*<\/c>/g);var d=[];if(null==a)return d;for(var e=0,g=a.length;e<g;e++){var k=[];this.errorFound();k.attrs=[];for(var f,h,m=a[e].match(b),l=0;l<m.length;l++)f=m[l].split(/=/),h=f[1].replace(/"/g,""),k.attrs[f[0]]="true"!=h?parseInt(h):
h;k.suggestions=[];f=a[e].replace(/<[^>]*>/g,"").split(c);for(h=0;h<f.length;h++)""!=f[h]&&k.suggestions.push(f[h]);d.push(k)}return d};this.processData=function(a){this.results=this.parseResult(a);this.results.length&&(this.showErrorsInIframe(),this.resumeEditingState())};this.createErrorWindow=function(){this.error_window=document.createElement("div");$(this.error_window).addClass("googie_window popupmenu").attr("googie_action_btn","1")};this.isErrorWindowShown=function(){return $(this.error_window).is(":visible")};
...
n=function(){""!=p.value&&(c.isDefined(a.old_value)||c.saveOldValue(a,a.innerHTML),c.updateOrginalText(m,a.innerHTML,p.value,b),$(a).attr("is_corrected",!0).css("color","green").html(p.value),c.hideErrorWindow());return!1};$(p).width(120).css({margin:0,padding:0});$(p).val(a.innerHTML).attr("googie_action_btn","1");$(f).css("cursor","default").attr("googie_action_btn","1");$(h).attr("src",this.img_dir+"ok.gif").width(32).height(16).css({cursor:"pointer","margin-left":"2px","margin-right":"2px"}).click(n);
...
$(this.spell_span).removeClass().addClass("googie_no_style");this.ignore||(this.use_focus&&($(this.focus_link_t).remove(),$(this.focus_link_b).remove()),$(this.edit_layer).remove(),$(this.text_area).show(),void 0!=this.el_scroll_top&&(this.text_area.scrollTop=this.el_scroll_top));this.checkSpellingState(!1)};this.createErrorLink=function(a,b){var c=document.createElement("span"),d=this,e=function(a){d.showErrorWindow(c,b);e=null;return!1};$(c).html(a).addClass("googie_link").click(e).removeAttr("is_corrected").attr({googie_action_btn:"1",
...
document.createElement("span");a=this.createSpellDiv();var b=this;this.custom_spellcheck_starter?$(a).bind("click",function(a){b.custom_spellcheck_starter()}):$(a).bind("click",function(a){b.spellCheck()});this.main_controller&&("left"==this.change_lang_pic_placement?$(this.spell_container).empty().append(this.switch_lan_pic).append(" ").append(a):$(this.spell_container).empty().append(a).append(" ").append(this.switch_lan_pic));this.spell_span=a};this.isDefined=function(a){return void 0!==a&&null!==
...
$(e).bind("click",c).bind("mouseover",this.item_onmouseover).bind("mouseout",this.item_onmouseout);e.appendChild(g);d.appendChild(e);return d};this.removeIndicator=function(a){window.rcmail&&rcmail.set_busy(!1,null,this.rc_msg_id)};this.appendIndicator=function(a){window.rcmail&&(this.rc_msg_id=rcmail.set_busy(!0,"checking"))};this.createFocusLink=function(a){var b=document.createElement("a");$(b).attr({href:"javascript:;",name:a});return b};this.item_onmouseover=function(a){"googie_list_revert"!=
this.className&&"googie_list_close"!=this.className?this.className="googie_list_onhover":this.parentNode.className="googie_list_onhover"};this.item_onmouseout=function(a){"googie_list_revert"!=this.className&&"googie_list_close"!=this.className?this.className="googie_list_onout":this.parentNode.className="googie_list_onout"}};
