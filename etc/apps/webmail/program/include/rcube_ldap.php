...
class rcube_ldap extends rcube_addressbook {
    ...
    private function _connect() {
        ...
        if (!function_exists('ldap_connect'))
            raise_error(array('code' => 100, 'type' => 'ldap',
                ...
                'message' => "No ldap support in this installation of PHP"), true, true);
    ...
    public function sasl_bind($authc, $pass, $authz = null) {
        ...
        if (!function_exists('ldap_sasl_bind')) {
            raise_error(array('code' => 100, 'type' => 'ldap',
                'file' => __FILE__, 'line' => __LINE__,
                'message' => "Unable to bind: ldap_sasl_bind() not exists"), true, true);
        }
        ...
        raise_error(array(
            ...
            'file' => __FILE__, 'line' => __LINE__,
            'message' => "Bind failed for authcid=$authc " . ldap_error($this->conn)), true);
    ...
    public function bind($dn, $pass) {
        ...
        raise_error(array(
            'code' => ldap_errno($this->conn), 'type' => 'ldap',
            'file' => __FILE__, 'line' => __LINE__,
            'message' => "Bind failed for dn=$dn: " . ldap_error($this->conn)), true);
    ...
    private function _list_group_members($dn, $entry, $attr, $count) {
        ...
        for ($i = 0; $i < $entry[$attr]['count']; $i++) {
            ...
            $result = @ldap_read($this->conn, $entry[$attr][$i], '(objectclass=*)', $attrib, 0, (int) $this->prop['sizelimit'], (int) $this->prop['timelimit']);
    ...
    private function _list_group_memberurl($dn, $entry, $count) {
        ...
        for ($i = 0; $i < $entry['memberurl']['count']; $i++) {
            ...
            $attrib = $count ? array('dn') : array_values($this->fieldmap);
            if ($result = @$func($this->conn, $m[1], $filter, $attrib, 0, (int) $this->prop['sizelimit'], (int) $this->prop['timelimit'])
            ) {
