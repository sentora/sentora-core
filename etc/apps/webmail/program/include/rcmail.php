...
class rcmail {
    ...
    public function memcache_failure($host, $port) {
        ...
        if (!$seen["$host:$port"]++) {
            ...
            raise_error(array('code' => 604, 'type' => 'db',
                ...
                'message' => "Memcache failure on host $host:$port"), true, false);
        }
    }
    ...
    public function get_address_book($id, $writeable = false) {
        ...
        if (!$contacts) {
            raise_error(array(
                ...
                'message' => "Addressbook source ($id) not found!"), true, true);
        }
    ...
    private function get_auth_hash($sess_id, $ts) {
        $auth_string = sprintf('rcmail*sess%sR%s*Chk:%s;%s', $sess_id, $ts, $this->config->get('ip_check') ? $_SERVER['REMOTE_ADDR'] : '***.***.***.***', $_SERVER['HTTP_USER_AGENT']);
