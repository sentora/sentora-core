...
class rcube_session {
        ...
        if ($config->get('session_storage', 'db') == 'memcache') {
            ...
            if ($this->memcache) {
                session_set_save_handler(
                        array($this, 'open'), array($this, 'close'), array($this, 'mc_read'), array($this, 'mc_write'), array($this, 'mc_destroy'), array($this, 'gc'));
            } else {
                raise_error(array('code' => 604, 'type' => 'db',
                    'line' => __LINE__, 'file' => __FILE__,
                    'message' => "Failed to connect to memcached. Please check configuration"), true, true);
            }
        } else {
            // set custom functions for PHP session management
            session_set_save_handler(
                    array($this, 'open'), array($this, 'close'), array($this, 'db_read'), array($this, 'db_write'), array($this, 'db_destroy'), array($this, 'db_gc'));
        }
    }
    ...
    public function db_write($key, $vars) {
        ...
        if ($oldvars !== false) {
            ...
            if ($newvars !== $oldvars) {
                $this->db->query(
                        sprintf("UPDATE %s SET vars=?, changed=%s WHERE sess_id=?", get_table_name('session'), $now), base64_encode($newvars), $key);
            ...
            $this->db->query(
                    sprintf("INSERT INTO %s (sess_id, vars, ip, created, changed) " .
                            "VALUES (?, ?, ?, %s, %s)", get_table_name('session'), $now, $now), $key, base64_encode($vars), (string) $this->ip);
        }
    ...
    public function db_destroy($key) {
        $this->db->query(
                sprintf("DELETE FROM %s WHERE sess_id = ?", get_table_name('session')), $key);
    ...
    public function db_gc($maxlifetime) {
        ...
        $this->db->query(
                sprintf("DELETE FROM %s WHERE changed < %s", get_table_name('session'), $this->db->fromunixtime(time() - $maxlifetime)));
    ...
    public function mc_read($key) {
        ...
        if ($this->mc_debug)
            write_log('memcache', "get($key): " . strlen($value));
    ...
    public function mc_destroy($key) {
        ...
        if ($this->mc_debug)
            write_log('memcache', "delete($key): " . ($ret ? 'OK' : 'ERR'));
        return $ret;
    }
