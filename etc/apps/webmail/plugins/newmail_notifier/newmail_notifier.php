...
class newmail_notifier extends rcube_plugin {
        ...
        foreach (array('basic', 'desktop', 'sound') as $type) {
            ...
            if (!in_array($key, $dont_override)) {
                ...
                $content = $input->show($this->rc->config->get($key))
                        . ' ' . html::a(array('href' => '#', 'onclick' => 'newmail_notifier_test_' . $type . '()'), $this->gettext('test'));
    ...
    function notify($args) {
        ...
        if ($basic || $sound || $desktop) {
            $this->rc->output->command('plugin.newmail_notifier', array('basic' => $basic, 'sound' => $sound, 'desktop' => $desktop));
        }
