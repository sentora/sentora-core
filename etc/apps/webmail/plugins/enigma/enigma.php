    ...
    function status_message($p) {
        ...
        if (isset($this->engine->decryptions[$part_id])) {
            ...
            if ($status instanceof enigma_error) {
                ...
                if ($code == enigma_error::E_KEYNOTFOUND)
                    $msg = Q(str_replace('$keyid', enigma_key::format_id($status->getData('id')), $this->gettext('decryptnokey')));
        ...
        if (isset($this->engine->signed_parts[$part_id])
            ...
            if ($sig instanceof enigma_signature) {
                ...
                $attrib['class'] = 'enigmawarning';
                $msg = Q(str_replace('$keyid', enigma_key::format_id($sig->getData('id')), $this->gettext('signokey')));
            } else {
                $attrib['class'] = 'enigmaerror';
...
//            $msg .= '<br /><pre>'.$sig->body.'</pre>';
    ...
    function message_output($p) {
        ...
        foreach ($this->keys_parts as $part) {
            ...
            $p['content'] .= html::p(array('style' => $style), html::a(array(
                                'href' => "#",
                                'onclick' => "return " . JS_OBJECT_NAME . ".enigma_import_attachment('" . JQ($part) . "')",
                                'title' => $this->gettext('keyattimport')), html::img(array('src' => $this->url('skins/default/key_add.png'), 'style' => "vertical-align:middle")))
